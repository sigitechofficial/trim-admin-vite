...
export default function BookingDetails() {
  const { data } = GetAPI(
    `admin/appointment-detail/${JSON.parse(
      localStorage.getItem("bookingDetailsID")
    )}`,
    "bookings"
  );
          ...
          <div className="bg-white p-5 rounded-lg space-y-6">
            <div className="grid xl:grid-cols-2 gap-5">
              ...
              <NewSalonProfileCard
                ...
                salonAverageRating={
                  data?.data?.appointments?.salonDetail?.salonAverageRating
                }
            ...
            <div className="bg-themeGray rounded-2xl p-4 lg:p-6 space-y-8">
              {data?.data?.appointments?.jobs?.map((job, i) => (
                <div key={i}>
                  <div className="flex justify-between w-full">
                    <div className="flex gap-x-4">
                      <img
                        src={
                          job?.employee?.user?.image
                            ? `${BASE_URL}${job?.employee?.user?.image}`
                            : `/images/defaultProfileImg.webp`
                        } // agar profile ati to theek nahi to defaultProfileImg from public
                        alt="user"
                        className="w-24 h-16 object-cover sm:object-contain"
                      />
                      <div className="space-y-2">
                        <h2 className="text-xl sm:text-2xl font-medium font-workSans">
                          {/* Jimmy */}
                          {job?.employee?.user
                  ...
                  <div className="space-y-4">
                    <h4 className="text-lg sm:text-xl font-workSans font-medium">
                      Service
                    </h4>
                    <div className="border border-inputBorder rounded-lg flex justify-between p-4">
                      <div className="font-workSans font-medium">
                        ...
                        <span className="text-themeLightGray">
                          {" "}
              ...
              {data?.data?.appointments?.jobs.length > 0 ? (
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <p className="font-workSans font-medium">
                      $ {data?.data?.appointments?.discount}
                    </p>
                  </div>
                  <div className="flex justify-between">
                    <h2 className="text-themeLightGray font-workSans font-medium">
                    ...
                    <p className="font-workSans font-medium">
                      $ {data?.data?.appointments?.subTotal}
                    </p>
                  </div>
                  <div className="flex justify-between">
                    <h2 className="text-themeLightGray font-workSans font-medium">
